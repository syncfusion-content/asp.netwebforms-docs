...
{% highlight html %}
    ...
    <body>
    <form runat="server">
            <ej:PivotGauge ID="MyPivotGauge1" runat="server" Url="/RelationalGauge"  ClientIDMode="Static">
            </ej:PivotGauge>
    </form>
    </body>
...
N> The above "Default.aspx" contains WebAPI URL, which is "/RelationalGauge". If WCF service is used as endpoint, the URL would look like "/RelationalGaugeService.svc".
...
To add a WebAPI controller in an existing Empty Web Application, right-click on the project in Solution Explorer and select **Add > New Item**. In the **Add New Item** window, select **WebAPI Controller Class** and name it as **“RelationalGaugeController.cs”**, click **Add**.
